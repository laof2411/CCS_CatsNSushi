...
namespace VrGamesDev
{
    ...
    public class VRG_RemoteBool
    {
        ...
        public VRG_RemoteBool(string keyLocal, bool valueLocal)
        {
            this.key = keyLocal;
    ...
    public class VRG_RemoteInt
    {
        ...
        public VRG_RemoteInt(string keyLocal, int valueLocal)
        {
            this.key = keyLocal;
    ...
    public class VRG_RemoteFloat
    {
        ...
        public VRG_RemoteFloat(string keyLocal, float valueLocal)
        {
            this.key = keyLocal;
    ...
    public class VRG_RemoteString
    {
        ...
        //public string _name; // Backing field for name, not strictly necessary here but shown for completeness
        ...
        public VRG_RemoteString(string keyLocal, string valueLocal)
        {
            this.key = keyLocal;
    ...
    public class VRG_Remote : VRG_Base
    {
        ...
        public static VRG_Remote Instance; private void Awake()
        {
            ...
            if (VRG_Remote.Instance == null)
            {
                ...
                this.Logs
                (
                    "Remote Config Data: <b>" + "REMOTE_CONFIG_INSTALLED" + "</b>",
                    "VRG_Remote->Awake()",
                    ENUM_Verbose.ALL
                );
        ...
        private IEnumerator Awake_IEnumerator()
        {
            ...
            {
                sLogsTemp = VRG_Remote.Instance.AddBool(child.key, child.value);
            ...
            {
                sLogsTemp = VRG_Remote.Instance.AddInt(child.key, child.value);
                if (sLogsTemp != string.Empty)
                {
                    sLoggable = true;
                    sLogsBlock += sLogsTemp;
            ...
            {
                sLogsTemp = VRG_Remote.Instance.AddFloat(child.key, child.value);
                if (sLogsTemp != string.Empty)
                {
                    sLoggable = true;
                    sLogsBlock += sLogsTemp;
            ...
            {
                sLogsTemp = VRG_Remote.Instance.AddString(child.key, child.value);
                if (sLogsTemp != string.Empty)
                {
                    sLoggable = true;
                    sLogsBlock += sLogsTemp;
        ...
        async Task Start()
        {
            ...
            await RemoteConfigService.Instance.FetchConfigsAsync(new VRG_Remote_UserAttributes(), new VRG_Remote_AppAttributes());
        ...
        private IEnumerator ReadRemoteServer()
        {
            ...
            foreach (VRG_RemoteBool child in this.m_Bools)
            {
                this.m_BoolsLocal.Add(new VRG_RemoteBool(child.key, child.value));
                this.m_BoolsRemote.Add(new VRG_RemoteBool(child.key, child.value));
            }
            foreach (VRG_RemoteInt child in this.m_Ints)
            {
                this.m_IntsLocal.Add(new VRG_RemoteInt(child.key, child.value));
                this.m_IntsRemote.Add(new VRG_RemoteInt(child.key, child.value));
            }
            foreach (VRG_RemoteFloat child in this.m_Floats)
            {
                this.m_FloatsLocal.Add(new VRG_RemoteFloat(child.key, child.value));
                this.m_FloatsRemote.Add(new VRG_RemoteFloat(child.key, child.value));
            }
            foreach (VRG_RemoteString child in this.m_Strings)
            {
                this.m_StringsLocal.Add(new VRG_RemoteString(child.key, child.value));
                this.m_StringsRemote.Add(new VRG_RemoteString(child.key, child.value));
            }
            ...
            foreach (VRG_RemoteBool child in this.m_Bools)
            {
                sLogs += "\n- BOOL | <b>" + child.key + "</b> = " + child.value;
            }
            ...
            foreach (VRG_RemoteFloat child in this.m_Floats)
            {
                sLogs += "\n- FLOAT | <b>" + child.key + "</b> = " + child.value;
            }
            ...
            foreach (VRG_RemoteString child in this.m_Strings)
            {
                sLogs += "\n- STRING | <b>" + child.key + "</b> = " + child.value;
            }
            ...
            string[] getKeys = RemoteConfigService.Instance.appConfig.GetKeys();

            // ... and check them all
            foreach (string child in getKeys)
            {
                // if it does exist in the arrays
                i = this.m_BoolsRemote.FindIndex((x) => x.key == child);

                // just if it exist and is new
                if (i >= 0)
                {
                    // assign the  value from the remote
                    this.m_BoolsRemote[i].value = RemoteConfigService.Instance.appConfig.GetBool(child);
                ...
                // if it does exist in the arrays
                i = this.m_IntsRemote.FindIndex((x) => x.key == child);
                // just if it exist and is new
                if (i >= 0)
                {
                    // assign the  value from the remote
                    this.m_IntsRemote[i].value = RemoteConfigService.Instance.appConfig.GetInt(child);
                ...
                // if it does exist in the arrays
                i = this.m_FloatsRemote.FindIndex((x) => x.key == child);
                // just if it exist and is new
                if (i >= 0)
                {
                    // assign the  value from the remote
                    this.m_FloatsRemote[i].value = RemoteConfigService.Instance.appConfig.GetFloat(child);
                ...
                // if it does exist in the arrays
                i = this.m_StringsRemote.FindIndex((x) => x.key == child);
                // just if it exist and is new
                if (i >= 0)
                {
                    // assign the  value from the remote
                    this.m_StringsRemote[i].value = RemoteConfigService.Instance.appConfig.GetString(child);
        ...
        public static bool GetBool(string valueLocal, bool bhelLocal)
        {
            ...
            if (Instance != null)
            {
                ...
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    // if it does exist in the arrays
                    int i = Instance.m_Bools.FindIndex((x) => x.key == valueLocal);
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            bRegresa = RemoteConfigService.Instance.appConfig.GetBool(valueLocal);
        ...
        public static int GetInt(string valueLocal, bool bhelLocal)
        {
            ...
            if (Instance != null)
            {
                ...
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    // if it does exist in the arrays
                    int i = Instance.m_Ints.FindIndex((x) => x.key == valueLocal);
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            iRegresa = RemoteConfigService.Instance.appConfig.GetInt(valueLocal);
        ...
        public static float GetFloat(string valueLocal, bool bhelLocal)
        {
            ...
            if (Instance != null)
            {
                ...
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    // if it does exist in the arrays
                    int i = Instance.m_Floats.FindIndex((x) => x.key == valueLocal);
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            fRegresa = RemoteConfigService.Instance.appConfig.GetFloat(valueLocal);
        ...
        public static string GetString(string valueLocal, bool bhelLocal)
        {
            ...
            if (Instance != null)
            {
                ...
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    // if it does exist in the arrays
                    int i = Instance.m_Strings.FindIndex((x) => x.key == valueLocal);
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            sRegresa = RemoteConfigService.Instance.appConfig.GetString(valueLocal);
            ...
            if (Instance != null)
            {
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            Instance.AddBool(valueLocal, RemoteConfigService.Instance.appConfig.GetBool(valueLocal));
            ...
            if (Instance != null)
            {
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            Instance.AddInt(valueLocal, RemoteConfigService.Instance.appConfig.GetInt(valueLocal));
            ...
            if (Instance != null)
            {
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            Instance.AddFloat(valueLocal, RemoteConfigService.Instance.appConfig.GetFloat(valueLocal));
            ...
            if (Instance != null)
            {
                if (valueLocal.Trim() != string.Empty)
                {
                    ...
                    if (i < 0)
                    {
                        ...
                        if (RemoteConfigService.Instance.appConfig.HasKey(valueLocal))
                        {
                            Instance.AddString(valueLocal, RemoteConfigService.Instance.appConfig.GetString(valueLocal));
        ...
        public string AddBool(string dataLocal, bool valueLocal)
        {
...
#if REMOTE_CONFIG_INSTALLED
            // ... and the key exists
            if (RemoteConfigService.Instance.appConfig.HasKey(dataLocal))
            {
                // update the value
                sValueRemote = RemoteConfigService.Instance.appConfig.GetBool(dataLocal);
            ...
            // if it does exist in the arrays
            int i = this.m_Bools.FindIndex((x) => x.key == dataLocal);
        ...
        public string AddInt(string dataLocal, int valueLocal)
        {
            ...
            {
                // update the value
                sValueRemote = RemoteConfigService.Instance.appConfig.GetInt(dataLocal);
            ...
            // if it does exist in the arrays
            int i = this.m_Ints.FindIndex((x) => x.key == dataLocal);
        ...
        public string AddFloat(string dataLocal, float valueLocal)
        {
            ...
            {
                // update the value
                sValueRemote = RemoteConfigService.Instance.appConfig.GetFloat(dataLocal);
            ...
            // if it does exist in the arrays
            int i = this.m_Floats.FindIndex((x) => x.key == dataLocal);
        ...
        public string AddString(string dataLocal, string valueLocal)
        {
            ...
            {
                // update the value
                sValueRemote = RemoteConfigService.Instance.appConfig.GetString(dataLocal);
            ...
            // if it does exist in the arrays
            int i = this.m_Strings.FindIndex((x) => x.key == dataLocal);
